...
server.registerTool('list_projects', {
    title: 'List projects',
    description: 'List all projects in the issue tracker with their id, title, category and leader. ' +
        'Call this before creating an issue to find the correct projectId — never guess a project id.',
    inputSchema: {},
}, async () => run(() => apiFetch('GET', '/projects')));
server.registerTool('search_issues', {
    title: 'Search issues',
    description: 'Search and filter issues by project, status, priority and/or assignee. ' +
...
}, async ({ projectId, status, priority, assigneeUuid, page, size }) => run(() => {
    ...
    if (assigneeUuid)
        params.set('assigneeUuid', assigneeUuid);
...
server.registerTool('get_issue', {
    ...
    description: 'Get the full details of one issue by its numeric id, including its comments (threaded, with soft-deleted ones marked) and attachment metadata.',
    inputSchema: {
...
server.registerTool('search_users', {
    title: 'Search users',
    description: 'Search users by name, username or email. Returns uuid, name, username and role. ' +
...
}, async ({ query }) => run(() => apiFetch('GET', `/users/search?q=${encodeURIComponent(query)}&size=50`)));
/* ------------------------------------------------------------ write tools */
server.registerTool('create_issue', {
    title: 'Create issue',
    description: 'Create a new issue in a project. The issue is created with status OPEN and the service account as creator. ' +
...
server.registerTool('update_issue_status', {
    ...
    description: 'Change the status of an issue. Valid statuses: OPEN, IN_PROGRESS, DONE. ' +
...
}, async ({ issueId, status }) => run(() => apiFetch('PATCH', `/issues/${issueId}/status`, { status })));
server.registerTool('assign_issue', {
    title: 'Assign issue',
    description: 'Replace the assignees of an issue with the given user uuids (empty array unassigns everyone). ' +
...
}, async ({ issueId, assignedUuids }) => run(async () => {
    // PUT /issues/{id} replaces the whole resource, so merge with current values.
    const current = (await apiFetch('GET', `/issues/${issueId}`));
...
server.registerTool('add_comment', {
    title: 'Add comment',
    description: 'Add a comment to an issue, optionally as a reply to an existing comment (parentCommentId). ' +
...
}, async ({ issueId, content, title, parentCommentId }) => run(() => apiFetch('POST', `/issues/${issueId}/comments`, {
    title: title ?? '',
    content,
    parentCommentId,
})));
